...
# Set the unified experiment name
mlflow.set_experiment("wdr_tourism_recommendation_models")

def calculate_ndcg(y_true, y_pred, k=10):
    """Calculate NDCG@K score."""
    try:
        # Convert to binary relevance (1 if rating >= 4, 0 otherwise)
...
def train_collaborative_filtering():
    ...
    logging.info("Training Collaborative Filtering (SVD)...")
    ...
    with mlflow.start_run(run_name="collaborative_filtering_svd") as run:
        # Train SVD
        ...
        # Register model with proper signature
        try:
            signature = mlflow.models.infer_signature(train_matrix.values, predicted_ratings)
            mlflow.sklearn.log_model(
                sk_model=svd,
                artifact_path="model",
                signature=signature,
                registered_model_name="wdr-tourism-collaborative-filtering"
            )
            logging.info("✅ Model registered successfully")
        except Exception as e:
            logging.warning(f"Model registration failed, but metrics logged: {e}")
        
        logging.info(f"Collaborative Filtering - RMSE: {rmse:.4f}, MAE: {mae:.4f}")
        return rmse, mae

def train_popularity_baseline():
    """Train popularity-based baseline model."""
    logging.info("Training Popularity Baseline...")
    ...
    with mlflow.start_run(run_name="popularity_baseline") as run:
        ...
        for _, row in test_df.iterrows():
            ...
            if place_id in place_avg_ratings:
                pred_rating = place_avg_ratings[place_id]
            else:
        ...
        logging.info(f"Popularity Baseline - RMSE: {rmse:.4f}, MAE: {mae:.4f}")
        return rmse, mae

def train_content_based():
    """Train content-based model."""
    logging.info("Training Content-Based Filtering...")
    ...
    with mlflow.start_run(run_name="content_based_tfidf") as run:
        ...
        desc_features = tfidf.fit_transform(place_features['place_description'].fillna(''))
        
        # Categorical features
        le_category = LabelEncoder()
        le_city = LabelEncoder()
        place_features['category_encoded'] = le_category.fit_transform(place_features['place_category'])
        place_features['city_encoded'] = le_city.fit_transform(place_features['place_city'])
        
        # Numerical features
        scaler = StandardScaler()
        numerical_features = ['place_price', 'place_average_rating', 'place_visit_duration_minutes']
        place_features[numerical_features] = scaler.fit_transform(place_features[numerical_features])
        ...
        for user_id in train_df['user_id'].unique():
            ...
            if place_indices:
                ...
                weights = (weighted_ratings - weighted_ratings.min() + 1) / (weighted_ratings.max() - weighted_ratings.min() + 1)
        ...
        if predictions:
            ...
            # Register TF-IDF vectorizer
            try:
                mlflow.sklearn.log_model(
                    sk_model=tfidf,
                    artifact_path="tfidf_vectorizer",
                    registered_model_name="wdr-tourism-content-based-tfidf"
                )
                
                mlflow.sklearn.log_model(
                    sk_model=scaler,
                    artifact_path="feature_scaler",
                    registered_model_name="wdr-tourism-content-based-scaler"
                )
                logging.info("✅ Content-based models registered successfully")
            except Exception as e:
                logging.warning(f"Model registration failed, but metrics logged: {e}")
            
            logging.info(f"Content-based - RMSE: {rmse:.4f}, MAE: {mae:.4f}, F1: {f1:.4f}")
            return rmse, mae
...
def train_neural_collaborative_filtering():
    ...
    logging.info("Training Neural Collaborative Filtering...")
    
    df = load_data_from_clickhouse()
    df, user_encoder, item_encoder = create_user_item_encoders(df)
    ...
    with mlflow.start_run(run_name="neural_collaborative_filtering") as run:
        # Build NCF model
        ...
        try:
            mlflow.tensorflow.log_model(
                model=model,
                artifact_path="model",
                registered_model_name="wdr-tourism-neural-cf"
            )
            logging.info("✅ Neural CF model registered successfully")
        except Exception as e:
            logging.warning(f"Model registration failed, but metrics logged: {e}")
        
        logging.info(f"Neural CF - RMSE: {rmse:.4f}, MAE: {mae:.4f}, NDCG@10: {ndcg_10:.4f}")
        return rmse, mae

def train_advanced_hybrid_model():
    """Train advanced hybrid model with enhanced feature engineering."""
    logging.info("Training Advanced Hybrid Model with Enhanced Features...")
    
    df = load_data_from_clickhouse()
    train_df, test_df = train_test_split(df, test_size=0.2, random_state=42)
    
    with mlflow.start_run(run_name="advanced_hybrid_model") as run:
        ...
        try:
            ...
            mlflow.sklearn.log_model(
                ...
                artifact_path="model",
                signature=signature,
                registered_model_name="wdr-tourism-advanced-hybrid-gb"
            )
            
            # Also register the scaler
            mlflow.sklearn.log_model(
                sk_model=scaler,
                artifact_path="scaler",
                registered_model_name="wdr-tourism-advanced-hybrid-scaler"
            )
            logging.info("✅ Advanced hybrid models registered successfully")
        except Exception as e:
            logging.warning(f"Model registration failed, but metrics logged: {e}")
        
        logging.info(f"Advanced Hybrid - RMSE: {rmse:.4f}, MAE: {mae:.4f}, F1: {f1:.4f}, NDCG@10: {ndcg_10:.4f}")
        return rmse, mae

def train_hybrid_model():
    """Train original hybrid model for comparison."""
    logging.info("Training Original Hybrid Model...")
    
    df = load_data_from_clickhouse()
    train_df, test_df = train_test_split(df, test_size=0.2, random_state=42)
    
    with mlflow.start_run(run_name="hybrid_random_forest") as run:
        # Simple hybrid approach - use statistical features
        features = []
        targets = []
        
        # Group by user and place to create features
        user_stats = train_df.groupby('user_id')['user_rating'].agg(['mean', 'std', 'count']).fillna(0)
        place_stats = train_df.groupby('place_id')['user_rating'].agg(['mean', 'std', 'count']).fillna(0)
        global_mean = train_df['user_rating'].mean()
        ...
        for _, row in train_df.iterrows():
            ...
            # User features
            user_mean = user_stats.loc[user_id, 'mean'] if user_id in user_stats.index else global_mean
            user_std = user_stats.loc[user_id, 'std'] if user_id in user_stats.index else 1.0
            user_count = user_stats.loc[user_id, 'count'] if user_id in user_stats.index else 0
            
            # Place features
            place_mean = place_stats.loc[place_id, 'mean'] if place_id in place_stats.index else global_mean
            place_std = place_stats.loc[place_id, 'std'] if place_id in place_stats.index else 1.0
            place_count = place_stats.loc[place_id, 'count'] if place_id in place_stats.index else 0
            ...
            feature_vector = [
                ...
                place_price, place_rating, place_duration, user_age,
                global_mean
            ]
        ...
        # Log parameters
        mlflow.log_params({
            "model_type": "hybrid_random_forest",
            "n_estimators": 100,
            "max_depth": 10,
            "n_features": len(features[0]),
            "global_mean": global_mean
        ...
        mlflow.log_metrics({
            ...
            "recall": recall,
            "f1_score": f1,
            "feature_importance_mean": np.mean(rf_model.feature_importances_)
        })
        
        # Tag the run
        mlflow.set_tags({
            "model_type": "hybrid",
            "algorithm": "Random Forest + Statistical Features",
            "framework": "scikit-learn"
        })
        
        # Register model
        try:
            signature = mlflow.models.infer_signature(np.array(features), predictions)
            mlflow.sklearn.log_model(
                sk_model=rf_model,
                artifact_path="model",
                signature=signature,
                registered_model_name="wdr-tourism-hybrid-random-forest"
            )
            logging.info("✅ Model registered successfully")
        except Exception as e:
            logging.warning(f"Model registration failed, but metrics logged: {e}")
        
        logging.info(f"Hybrid Model - RMSE: {rmse:.4f}, MAE: {mae:.4f}, F1: {f1:.4f}")
        return rmse, mae
...
def train_ensemble_model():
    ...
    with mlflow.start_run(run_name="ensemble_model") as run:
        ...
        logging.info("Training SVD component...")
        svd = TruncatedSVD(n_components=30, random_state=42)
        user_factors = svd.fit_transform(user_item_matrix)
        item_factors = svd.components_
        
        # Prepare feature-based model
        logging.info("Training feature-based component...")
        features = []
        targets = []
        ...
        for _, row in train_df.iterrows():
            ...
            feature_vector = [
                user_mean, user_count, place_mean, place_count,
                row['place_price'], row['place_average_rating'],
                row['place_visit_duration_minutes'], row['user_age']
            ]
            features.append(feature_vector)
            targets.append(row['user_rating'])
        ...
        for _, row in test_df.iterrows():
            ...
            if user_id in user_item_matrix.index:
                ...
                if place_id in user_item_matrix.columns:
                    ...
                    svd_pred = np.dot(user_factors[user_idx], item_factors[:, place_idx])
            ...
            popularity_pred = place_mean if place_id in place_stats.index else global_mean
            predictions_list.append(popularity_pred)
            
            # Ensemble prediction (weighted average)
            if predictions_list:
                weights = [0.4, 0.4, 0.2]  # SVD, Ridge, Popularity
                weighted_pred = np.average(predictions_list[:len(weights)], weights=weights[:len(predictions_list)])
        ...
        try:
            mlflow.sklearn.log_model(
                ...
                registered_model_name="wdr-tourism-ensemble-svd"
            )
            mlflow.sklearn.log_model(
                sk_model=ridge_model,
                artifact_path="ridge_model", 
                registered_model_name="wdr-tourism-ensemble-ridge"
            )
            logging.info("✅ Ensemble models registered successfully")
        except Exception as e:
            logging.warning(f"Model registration failed, but metrics logged: {e}")
        
        logging.info(f"Ensemble Model - RMSE: {rmse:.4f}, MAE: {mae:.4f}, F1: {f1:.4f}, NDCG@10: {ndcg_10:.4f}")
        return rmse, mae

def main():
    """Run all models in unified experiment including enhanced models."""
    logging.info("Starting enhanced unified experiment with all recommendation models...")
    
    results = {}
    
    # Original models
    try:
        rmse, mae = train_collaborative_filtering()
        results['Collaborative Filtering'] = {'RMSE': rmse, 'MAE': mae}
    except Exception as e:
        logging.error(f"Collaborative filtering failed: {e}")
        results['Collaborative Filtering'] = {'RMSE': float('inf'), 'MAE': float('inf')}
    
    try:
        rmse, mae = train_popularity_baseline()
        results['Popularity Baseline'] = {'RMSE': rmse, 'MAE': mae}
    except Exception as e:
        logging.error(f"Popularity baseline failed: {e}")
        results['Popularity Baseline'] = {'RMSE': float('inf'), 'MAE': float('inf')}
    
    try:
        rmse, mae = train_content_based()
        results['Content-Based'] = {'RMSE': rmse, 'MAE': mae}
    except Exception as e:
        logging.error(f"Content-based failed: {e}")
        results['Content-Based'] = {'RMSE': float('inf'), 'MAE': float('inf')}
    
    try:
        rmse, mae = train_hybrid_model()
        results['Hybrid Model (Original)'] = {'RMSE': rmse, 'MAE': mae}
    except Exception as e:
        logging.error(f"Original hybrid model failed: {e}")
        results['Hybrid Model (Original)'] = {'RMSE': float('inf'), 'MAE': float('inf')}
    
    # Enhanced models
    try:
        rmse, mae = train_neural_collaborative_filtering()
        results['Neural Collaborative Filtering'] = {'RMSE': rmse, 'MAE': mae}
    except Exception as e:
        logging.error(f"Neural collaborative filtering failed: {e}")
        results['Neural Collaborative Filtering'] = {'RMSE': float('inf'), 'MAE': float('inf')}
    
    try:
        rmse, mae = train_advanced_hybrid_model()
        results['Advanced Hybrid Model'] = {'RMSE': rmse, 'MAE': mae}
    except Exception as e:
        logging.error(f"Advanced hybrid model failed: {e}")
        results['Advanced Hybrid Model'] = {'RMSE': float('inf'), 'MAE': float('inf')}
    
    try:
        rmse, mae = train_ensemble_model()
        results['Ensemble Model'] = {'RMSE': rmse, 'MAE': mae}
    except Exception as e:
        logging.error(f"Ensemble model failed: {e}")
        results['Ensemble Model'] = {'RMSE': float('inf'), 'MAE': float('inf')}
    
    # Print comparison
    logging.info("\n" + "="*80)
    logging.info("ENHANCED UNIFIED EXPERIMENT RESULTS")
    logging.info("="*80)
    ...
    for model_name, metrics in results.items():
        ...
        if rmse != float('inf'):
            successful_models += 1
        logging.info(f"{model_name:35} | {status} | RMSE: {rmse:.4f} | MAE: {mae:.4f}")
    
    logging.info("="*80)
    logging.info(f"Successful models: {successful_models}/{len(results)}")
    logging.info(f"Experiment: wdr_tourism_recommendation_models")
    logging.info(f"MLflow URI: {MLFLOW_TRACKING_URI}")
    ...
    enhanced_models = ['Neural Collaborative Filtering', 'Advanced Hybrid Model', 'Ensemble Model']
    ...
    if enhanced_working > 0:
        logging.info("✨ Successfully enhanced the recommendation system with:")
    ...
    logging.info(f"🎯 MLOps tracking: All experiments logged to MLflow")
